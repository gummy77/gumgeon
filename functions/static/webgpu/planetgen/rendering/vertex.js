export const vertexWGSL = /* wgsl */`
    ...
    fn vert(
        ...
        var cellVertexPosition: vec3f = vertexPositions[(instanceIndex * 6) + inVertexIndex];
        ...
        cellVertexPosition *= 0.75;

        switch (displaySettings) {
            case(0) {
                var output: DisplayOutput = plates(cellHeight, cellPlateID);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(1) {
                var output: DisplayOutput = heightMap(cellHeight);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(2) {
                var output: DisplayOutput = tempuratureMap(cellHeight, cellTemperature);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(3) {
                var output: DisplayOutput = humidityMap(cellHeight, cellHumidity);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(4) {
                var output: DisplayOutput = boundaries(cellHeight, cellTemperature, cellHumidity, cellBoundaries);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(5) {
                var output: DisplayOutput = ores(cellHeight, cellTemperature, cellHumidity, cellOreType, cellOreRichness);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(6) {
                var output: DisplayOutput = Biomes(cellHeight, cellBiome);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            case(7) {
                var output: DisplayOutput = rendered(cellHeight, cellTemperature, cellHumidity);
                cellVertexPosition *= output.height;
                out.color = output.color;
            }
            default {
